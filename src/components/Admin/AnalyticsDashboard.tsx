...
function StatCard({ label, value, sub, accent, icon }: StatCardProps) {
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 flex items-start gap-3 shadow-sm">
...
function MiniBar({ label, count, max, colorClass }: MiniBarProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-20 text-xs font-medium text-slate-600 dark:text-slate-400 shrink-0">{label}</span>
      ...
      <span className="w-6 text-xs font-semibold text-slate-700 dark:text-slate-300 text-right shrink-0">{count}</span>
    </div>
  );
}
...
function TopItemsTable({ items }: { items: TopItem[] }) {
  if (items.length === 0) {
    return <p className="text-sm text-slate-400 dark:text-slate-500 py-4 text-center">No order data yet.</p>;
  }
  return (
    <div className="divide-y divide-slate-100 dark:divide-slate-700">
      {items.map((item, idx) => (
        <div key={item.name} className="flex items-center gap-3 py-2.5">
          ...
          <span className="flex-1 text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{item.name}</span>
          <span className="text-xs text-slate-500 dark:text-slate-400">{item.qty} sold</span>
          <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 w-16 text-right">
            ₹{item.revenue}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <svg width="72" height="72" viewBox="0 0 72 72">
        <circle cx="36" cy="36" r={r} fill="none" stroke="#e2e8f0" className="dark:stroke-slate-700" strokeWidth="7" />
        <circle
          cx="36"
          cy="36"
          r={r}
          ...
          strokeLinecap="round"
        />
        <text x="36" y="40" textAnchor="middle" className="text-xs" fontSize="13" fontWeight="700" fill="currentColor">
          <tspan className="fill-slate-800 dark:fill-slate-100">{rate}%</tspan>
        </text>
      </svg>
      <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Fulfillment</p>
    </div>
  );
}
...
export function AnalyticsDashboard({ orders, stats }: AnalyticsDashboardProps) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-3">Order Status Breakdown</h3>
      ...
      {/* Top items */}
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-sm">
        <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1">Top-Selling Items</h3>
        <p className="text-xs text-slate-400 dark:text-slate-500 mb-3">Ranked by units sold (non-cancelled orders)</p>
      ...
      {derived.categories.length > 0 && (
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-sm">
          <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-3">Revenue by Category</h3>
          <div className="space-y-2.5">
            {derived.categories.map((cat) => (
              <div key={cat.name} className="flex items-center gap-3">
                <span className="w-24 text-xs font-medium text-slate-600 dark:text-slate-400 shrink-0 truncate">{cat.name}</span>
                <div className="flex-1 h-2 rounded-full bg-slate-100 dark:bg-slate-700 overflow-hidden">
                  <div
                ...
                <span className="w-14 text-xs font-semibold text-slate-700 dark:text-slate-300 text-right shrink-0">₹{cat.rev}</span>
              </div>
            ))}
          </div>
        </div>
