 ...
 *
 * Shared presentational atoms used across the app.
 */
...
export function Spinner({ size = 'md', variant = 'primary' }: SpinnerProps) {
  return (
    <div
      role="status"
      aria-label="Loading"
      className={`${SPINNER_SIZE[size]} ${SPINNER_VARIANT[variant]} animate-spin rounded-full`}
    />
  );
}
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-sm font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 ${
        active
          ? 'border border-transparent bg-indigo-600 text-white shadow-md shadow-indigo-200'
...
export function CrowdIndicator({ level, percentage, label }: CrowdIndicatorProps) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 rounded-lg border px-3 py-1.5 ${cfg.bg} ${cfg.border}`}
    >
      <span className="relative flex h-2.5 w-2.5">
        <span
          className={`absolute inline-flex h-full w-full animate-ping rounded-full opacity-75 ${cfg.color}`}
          aria-hidden="true"
        />
        <span
      ...
      {percentage !== undefined && (
        <span className={`text-xs font-medium opacity-75 ${cfg.text}`}>
          {percentage}%
        </span>
      )}
    </div>
