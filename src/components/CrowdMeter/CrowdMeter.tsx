 ...
 *
 * Crowd-level gauge for CanteenCrowd. Semi-circular arc
...
export function CrowdMeter({ crowdStatus: externalData }: CrowdMeterProps) {
  ...
  if (isLoading || !data) {
    return (
      <div className="rounded-xl bg-white dark:bg-slate-800 p-5 border border-slate-100 dark:border-slate-700 shadow-sm">
        <Skeleton className="h-5 w-32 mb-4 dark:bg-slate-700" />
        <Skeleton className="h-[110px] w-full mb-4 dark:bg-slate-700" />
        <Skeleton className="h-4 w-48 mx-auto mb-3 dark:bg-slate-700" />
        <div className="grid grid-cols-3 gap-3 pt-3 border-t border-slate-100 dark:border-slate-700">
          <Skeleton className="h-12 dark:bg-slate-700" />
          <Skeleton className="h-12 dark:bg-slate-700" />
          <Skeleton className="h-12 dark:bg-slate-700" />
        </div>
      </div>
  ...
  return (
    <div
      className={`relative rounded-xl p-5 border border-slate-100 dark:border-slate-700 shadow-sm transition-colors duration-500 ${cfg.bg} ${cfg.darkBg}`}
    >
      ...
      <div className="flex items-center justify-between mb-1">
        {data.manualOverride ? (
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white/80 dark:bg-slate-700/80 border border-slate-200 dark:border-slate-600 px-2.5 py-1 text-[10px] font-medium text-slate-500 dark:text-slate-400">
        ...
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-full bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-500 dark:text-slate-400 transition-all duration-150 hover:bg-slate-50 dark:hover:bg-slate-600 hover:text-slate-700 dark:hover:text-slate-200 active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1"
        >
      ...
      <div className="flex flex-col items-center pt-2">
        ...
        >
          <defs>
            <linearGradient id="trackGradient" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor="#f1f5f9" className="dark:stop-color-[#334155]" />
              <stop offset="100%" stopColor="#e2e8f0" className="dark:stop-color-[#1e293b]" />
            </linearGradient>
          </defs>
        ...
        {/* Level label */}
        <p
          className={`-mt-2 text-2xl font-bold tracking-tight ${cfg.accent} ${cfg.darkAccent}`}
        >
          {cfg.label}
        </p>
      </div>
      ...
      {data.estimatedWaitMinutes > 0 && (
        <div className="mx-auto mt-3 flex w-fit items-center gap-2 rounded-full bg-white/80 dark:bg-slate-700/80 border border-slate-200/60 dark:border-slate-600/60 px-3.5 py-1.5 backdrop-blur-sm">
      ...
      {/* Description */}
      <p className="mt-3 text-center text-xs font-medium text-slate-600 dark:text-slate-400 leading-relaxed max-w-[260px] mx-auto">
        {cfg.description}
      </p>

      {/* Stats row */}
      <div className="mt-4 grid grid-cols-3 gap-2 border-t border-slate-200/60 dark:border-slate-700/60 pt-3 text-center">
        <div className="rounded-lg bg-white/60 dark:bg-slate-700/40 py-2">
