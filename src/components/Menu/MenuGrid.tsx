      ...
      {DIETARY_FILTERS.map(({ value, label, dot }) => {
        ...
        return (
          <button
            ...
            className={`flex flex-shrink-0 items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold transition-all duration-150 active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-300 dark:focus:ring-indigo-500 ${isActive
...
function MenuItemCard({ item, qty, onAdd, onUpdateQty }: CardProps) {
  return (
    <div
      className={`relative flex flex-col rounded-xl bg-white dark:bg-slate-800 p-3 border border-slate-100 dark:border-slate-700 shadow-sm transition-all duration-200 ${item.available
          ? 'hover:-translate-y-0.5 hover:shadow-md hover:border-slate-200 dark:hover:border-slate-600'
          : 'opacity-50 pointer-events-none'
        }`}
    >
      {!item.available && (
        <span className="absolute inset-0 z-10 flex items-center justify-center rounded-xl bg-white/70 dark:bg-slate-900/70 text-xs font-semibold text-slate-500 dark:text-slate-400 backdrop-blur-[1px]">
      ...
      <p className="line-clamp-2 mt-0.5 text-xs leading-relaxed text-slate-500 dark:text-slate-400">{item.description}</p>
      ...
      <div className="mt-auto flex items-center justify-between pt-3">
        <span className="text-base font-bold text-slate-900 dark:text-slate-100">₹{item.price}</span>

        {qty === 0 ? (
          <button
            onClick={onAdd}
            aria-label={`Add ${item.name} to cart`}
            className="flex items-center gap-1 rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition-all duration-150 hover:bg-indigo-700 active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 dark:focus:ring-offset-slate-800"
          >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-semibold transition-all duration-150 hover:bg-slate-200 dark:hover:bg-slate-600 active:scale-95"
            >
              −
            </button>
            <span className="w-5 text-center text-sm font-bold text-slate-900 dark:text-slate-100 tabular-nums">{qty}</span>
            <button
              onClick={() => onUpdateQty(1)}
...
export function MenuGrid({ items, isLoading, cart, onAdd, onUpdateQty }: MenuGridProps) {
  ...
  if (isLoading) {
    return (
      <div>
        <Skeleton className="h-10 w-full rounded-xl mb-3 dark:bg-slate-700" />
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <Skeleton key={i} className="h-8 w-24 flex-shrink-0 rounded-full dark:bg-slate-700" />
          ))}
        </div>
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-3">
    ...
    <div>
      ...
      <div className="relative mb-3">
        ...
        <input
          ...
          className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-2.5 pl-9 pr-9 text-sm text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-indigo-400 dark:focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900 transition-all"
        />
        {query && (
          <button
            onClick={() => setQuery('')}
      ...
      {activeFiltersCount > 0 && (
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs text-slate-500 dark:text-slate-400">
            <span className="font-semibold text-slate-700 dark:text-slate-300">{filtered.length}</span> item{filtered.length !== 1 ? 's' : ''} found
          </p>
          <button
