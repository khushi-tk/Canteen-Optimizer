...
export function CheckoutSummary({
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 dark:bg-slate-900">
      {/* Sticky header */}
      <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-slate-100 dark:border-slate-700 bg-white/90 dark:bg-slate-800/90 px-5 py-4 backdrop-blur-md">
        <button
          type="button"
          onClick={onBack}
          aria-label="Go back to menu"
          className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 transition-all duration-150 hover:bg-slate-200 dark:hover:bg-slate-600 active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 dark:focus:ring-offset-slate-800"
        >
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
          </svg>
        </button>
        <h1 className="text-lg font-semibold tracking-tight text-slate-900 dark:text-slate-100">Checkout</h1>
      </header>

      {/* Scrollable body */}
      <div className="flex-1 overflow-y-auto px-5 py-4 pb-32">
        {/* Order items card */}
        <div className="rounded-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-sm">
          <SectionHeader
            title="Your Order"
          ...
          {cart.map((ci, idx) => (
            <div
              key={ci.menuItem.id}
              className={`flex items-center gap-3 py-2.5 ${idx < cart.length - 1 ? 'border-b border-slate-100 dark:border-slate-700' : ''
                }`}
            >
          ...
          {/* Total */}
          <div className="mt-3 flex items-center justify-between rounded-lg bg-slate-50 dark:bg-slate-700 px-4 py-2.5">
            <span className="text-sm font-semibold text-slate-600 dark:text-slate-300">Total</span>
            <span className="text-lg font-bold text-slate-900 dark:text-slate-100 tabular-nums">₹{cartTotal}</span>
          </div>
        </div>
        ...
        {selectedSlot && (
          <div className="mt-4 flex items-center gap-2 rounded-full border border-indigo-100 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-950 px-4 py-2.5">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 mx-auto w-full max-w-[390px] sm:max-w-none border-t border-slate-100 dark:border-slate-700 bg-white/95 dark:bg-slate-800/95 px-5 pb-6 pt-3 backdrop-blur-md">
        ...
        <button
          ...
          disabled={!canPlace}
          className={`flex w-full items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 dark:focus:ring-offset-slate-800 ${canPlace
